...
def seed_database():
    ...
    with app.app_context():
        ...
        # Create standalone discussion posts about market trends (new section)
        print("Creating discussion posts...")
        market_discussion_titles = [
            ...
            "AI stocks to watch",
            "Crypto vs Stocks in current market",
            "Energy sector rotation happening?",
            "Small caps ready for comeback?",
            "International markets outlook"
        ]
        
        market_discussion_contents = [
            "I'm seeing strong signals that the market will continue its upward trajectory next quarter. Corporate earnings are exceeding expectations and consumer spending remains robust.",
            "Tech valuations seem stretched, but innovation continues to drive growth. I'm particularly interested in AI, cloud, and semiconductor companies right now.",
            "With the Fed likely pausing rate hikes, we might see a bull run in growth stocks. What's everyone's take?",
            "Despite recession fears, economic indicators remain mixed. Employment is strong, but inflation concerns persist. I'm staying invested but with increased cash reserves.",
            "Dividend aristocrats are outperforming in this volatile market. I've been adding to my positions in consumer staples and utilities.",
            "The energy transition is creating interesting opportunities. I'm balancing traditional energy with renewable plays to capture both current cash flow and future growth."
        ]
        
        # Create 6-10 discussion posts from random SEED users
        for _ in range(random.randint(6, 10)):
            poster = random.choice(seed_users)
            
            discussion_post = TradingPost(
                user_id=poster.id,
                title=random.choice(market_discussion_titles),
                content=random.choice(market_discussion_contents),
                trade_type='discussion',  # This is a discussion post, not tied to a trade
                is_public=True,
                # Add required parameters even though they're not relevant for discussions
                ticker='N/A',           # Not applicable for discussions
                quantity=0,             # Zero quantity for discussions
                price=0.0               # Zero price for discussions
            )
            
            # Post between 2-30 days ago
            discussion_post.created_at = now - timedelta(days=random.randint(2, 30))
            discussion_post.likes = random.randint(8, 35)
            discussion_post.dislikes = random.randint(0, 15)
            
            db.session.add(discussion_post)
            db.session.flush()
            
            # Add 3-8 comments to each discussion post
            commenters = random.sample(all_users, min(len(all_users), random.randint(3, 8)))
            
            for commenter in commenters:
                discussion_comment_options = [
                    "Great analysis! I've been thinking along similar lines.",
                    "I respectfully disagree. The economic indicators I'm watching suggest caution is warranted.",
                    "Interesting perspective. Have you considered the impact of geopolitical tensions?",
                    "This aligns with my research. I'd add that sector rotation will be key in the coming months.",
                    "What's your time horizon for this outlook? I'm more bearish short-term but bullish long-term.",
                    "The technical indicators support your view. The 200-day moving average is showing strong support.",
                    "Fundamentals still matter in this market. I'm focusing on companies with strong cash flows."
                ]
                
                comment = Comment(
                    post_id=discussion_post.id,
                    user_id=commenter.id,
                    content=random.choice(discussion_comment_options)
                )
                comment.created_at = discussion_post.created_at + timedelta(hours=random.randint(1, 72))
                db.session.add(comment)
                
                # 30% chance of nested replies
                if random.random() < 0.3:
                    # Get 1-3 other users to reply to this comment
                    reply_users = [u for u in all_users if u.id != commenter.id]
                    reply_count = min(len(reply_users), random.randint(1, 3))
                    
                    if reply_count > 0:
                        repliers = random.sample(reply_users, reply_count)
                        
                        for replier in repliers:
                            reply_options = [
                                "Good point! I hadn't considered that angle.",
                                "I see what you're saying, but the data suggests otherwise.",
                                "That's exactly what I've been thinking too!",
                                "Have you read the latest research on this? It might change your perspective.",
                                "Would love to hear more about your analysis on this."
                            ]
                            
                            # Create additional comments instead of nested replies
                            reply = Comment(
                                post_id=discussion_post.id,
                                user_id=replier.id,
                                content=f"@{commenter.first_name}: {random.choice(reply_options)}"  # Mentioning the user instead of using parent_comment_id
